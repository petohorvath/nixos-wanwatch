...
| `wanwatch_apply_op_errors_total` | counter | `group`, `op` | Errors per family-agnostic op. `op ∈ {conntrack_flush, ifindex_lookup}` in v1 — `state_write`, `hook`, and `rule_install` are reserved but not yet emitted. |

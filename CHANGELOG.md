...
- CI / tooling claims made good on: `.golangci.yml` now carries the curated check set referenced by `AGENTS.md` §Formatting and linting (`bodyclose`, `errcheck`, `errorlint`, `goconst`, `gocritic`, `gosec`, `govet`, `prealloc`, `revive`, `staticcheck`, `unparam`, `unused`). `.github/workflows/audit.yml` runs `govulncheck` + `vulnix` weekly + on release tag. Per-package coverage gates from PLAN §9.2 land as `checks.<system>.coverage`; the CI's daemon job builds it.
